        ...
        <div className="flex gap-2 p-1 bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto">
            {Object.values(AssetCategory).map((cat) => (
                <button
                    ...
                    className={`flex-1 min-w-[80px] py-2 px-2 rounded-md text-xs font-medium flex flex-col items-center justify-center gap-1 transition-all ${
                        category === cat 
                ...
                >
                    {getCategoryIcon(cat)}
                    <span>{cat === 'REAL_ESTATE' ? 'Immo' : cat === 'LIQUIDITY' ? 'Cash' : cat === 'INVESTMENT' ? 'Bourse' : 'Crypto'}</span>
                </button>
            ))}
        </div>
