...
export const SummaryCards: React.FC<SummaryCardsProps> = ({ totalIncome, totalExpense }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div className="bg-slate-800/50 backdrop-blur-md rounded-xl p-5 border border-slate-700/50 relative overflow-hidden group">
        ...
        <div className="text-xs text-emerald-500/70 mt-1">Flux entrants mensuels</div>
      </div>
      ...
      <div className="bg-slate-800/50 backdrop-blur-md rounded-xl p-5 border border-slate-700/50 relative overflow-hidden group">
        ...
        <div className="text-xs text-rose-500/70 mt-1">Flux sortants mensuels</div>
      </div>
      ...
      <div className="bg-slate-800/50 backdrop-blur-md rounded-xl p-5 border border-slate-700/50 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-slate-400 font-medium">Solde Mensuel</span>
        </div>
        <div className={`text-2xl font-bold ${balance >= 0 ? 'text-indigo-200' : 'text-orange-200'}`}>
            {format(balance)}
        </div>
        <div className="text-xs text-slate-500 mt-1">Capacité d'épargne théorique</div>
      </div>
    </div>
  );
};